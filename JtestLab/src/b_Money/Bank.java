...
public class Bank {
	...
	public void openAccount(String accountid) throws AccountExistsException {
		if (existsAccount(accountid)) {
			...
			// creating account with currency of this bank
	...
	public void deposit(String accountid, Money money) throws AccountDoesNotExistException {
		// ---- If account does not exist then this exception should be throwed. - add ! (negotiation mark) .
		if (!existsAccount(accountid)) {	
	...
	public void withdraw(String accountid, Money money) throws AccountDoesNotExistException {
		if (!accountlist.containsKey(accountid)) {
			...
			Account account = accountlist.get(accountid);
			account.withdraw(money);   // changing deposit to withdraw
		}
	}
	 ...
	 */
	public void transfer(String fromaccount, Bank tobank, String toaccount, Money amount) throws AccountDoesNotExistException {if (!existsAccount(fromaccount) || !tobank.existsAccount(toaccount)) {throw new AccountDoesNotExistException();
	}
	else {
	...
	public void transfer(String fromaccount, String toaccount, Money amount) throws AccountDoesNotExistException {
		...
		// It was transfer(from, this, from, amount), so transfering between same account
